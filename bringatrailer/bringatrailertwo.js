...
const getDataFromPage = async (url, browser, retryCount = 0) => {
    ...
    const loadData = async () => {
        try {
            ...
            const categoryArray = await page.$$eval(
                "div.group-item-wrap",
                (wraps) =>
                    wraps.reduce((acc, wrap) => {
                        ...
                        if (title && title.textContent.includes("Category")) {
                            acc.push(
                                title.textContent.split("Category")[1].trim()
                            );
                        }
                        return acc;
                    }, [])
            ...
            const auction_id = (
                await page.$eval(
                    "body > main > div > div.listing-intro",
                    (intro) => intro.getAttribute("data-listing-intro-id")
                )
            ).toString();
            ...
            const hasAllRequiredAttributes = requiredAttributeKeys.every(
                (key) => attributes.some((attr) => attr.key === key)
            );
            ...
            if (hasAllRequiredAttributes && hasOtherRequiredFields) {
                ...
                console.error(
                    `✖ Missing required fields for auction at URL: ${url}`
                );
                return false;
            }
        } catch (error) {
            ...
            console.error(
                `✖ Error processing auction at URL: ${url}, ${error}`
            );
            throw error;
        } finally {
            await page.close();
    ...
    for (const item of currentAuctionData) {
        try {
            ...
            if (existingAuction) {
                ...
                console.log(
                    `✔ Inserted new auction with ID ${item.auction_id}`
                );
                insertCount++;
            }
        } catch (error) {
...
const getFinalPriceFromPage = async (url, page, auctionId, retryCount = 0) => {
    ...
    try {
        ...
        console.log(
            `Current price for auction ID ${auctionId}: ${currentPrice}`
        );
        ...
        console.log(
            `Current price: ${currentPrice}, Final price: ${finalPrice}`
        );
...
const checkAndUpdateAuctionStatus = async (browser) => {
    ...
    console.log(
        `Fetched ${auctions.length} auctions for status and price check`
    );
        ...
        for (const auction of batch) {
            try {
                ...
                if (!auction.page_url || typeof auction.page_url !== "string") {
                    console.error(
                        `✖ Invalid URL for auctionID: ${auction.auction_id}`
                    );
                    unsuccessfulCount++;
                    continue;
                }
                ...
                if (
                    ...
                    console.log(
                        `✔ Updated status for auctionID: ${auction.auction_id}`
                    );
                }
